...
-- ==================== ANIMASI MODERN ====================
-- Enhanced circle click effect dengan particle feel
local function ModernCircleClick(Button, X, Y) 
    spawn(function() 
        ...
        -- Circle kedua untuk efek ripple ganda
        ...
        -- Animasi circle kedua (lambat, efek trail)
...
-- Smooth drag dengan spring effect
local function SmoothDraggable(topbarobject, object) 
    ...
    topbarobject.InputBegan:Connect(function(input) 
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then 
            ...
            -- Scale up effect saat mulai drag
            TweenService:Create(
                object, 
...
local ConfigSystem = { 
    ...
    Animations = {
        ...
        Style = "Modern" -- Classic, Modern, Bounce
    }
} 

-- Enhanced theme update dengan animasi
...
function ConfigSystem:UpdateAllThemeElements(newColor) 
    -- Update toggle elements dengan smooth transition
        ...
        if toggle and toggle:IsA("Frame") then 
            ...
            if toggleCircle then 
                ...
                if isOn then 
                    ...
                    if frameStroke then 
                        TweenService:Create(frameStroke, TweenInfo.new(0.3), {Color = newColor}):Play()
                    end 
                    if featureFrame then 
                        TweenService:Create(featureFrame, TweenInfo.new(0.3), {BackgroundColor3 = newColor}):Play()
                    end 
                    if toggleTitle then 
    ...
    for _, slider in pairs(self.SliderElements) do 
        if slider and slider:IsA("Frame") then 
            ...
            if circleStroke then 
                TweenService:Create(circleStroke, TweenInfo.new(0.3), {Color = newColor}):Play()
            end 
        end 
    end 
                ...
                if listScroll then 
                    for _, option in listScroll:GetChildren() do 
                        if option:IsA("Frame") and option.Name == "Option" then 
                            ...
                            if chooseFrame then 
                                TweenService:Create(chooseFrame, TweenInfo.new(0.3), {BackgroundColor3 = newColor}):Play()
                            end 
                            if chooseStroke then 
                                TweenService:Create(chooseStroke, TweenInfo.new(0.3), {Color = newColor}):Play()
                            end 
...
-- Enhanced notification dengan animasi modern
local function ModernNotify(NotifyConfig) 
    ...
    NotifyConfig.Icon = NotifyConfig.Icon or "rbxassetid://16932740082" -- Default icon
...
function Cyrus:CreateWindow(config) 
    ...
    local AnimationSpeed = config.AnimationSpeed or "Normal" -- Fast, Normal, Slow
    ...
    WindowIcon.Image = "rbxassetid://16932740082" 
    ...
    -- Window controls dengan efek hover
    ...
    -- Keyboard shortcut untuk minimize
    ...
    -- Draggable dengan smooth effect
    if Draggable then 
        SmoothDraggable(Top, DropShadowHolder) 
    end 
    
    -- Tab frame dengan efek modern
    ...
    -- Button creator dengan image support
    ...
    -- Toggle creator dengan animasi modern
    ...
    local function CreateDropdown(tabContainer, title, content, options, multi, defaultValue, callback, elementId) 
        ...
        local function CreateOption(optionName) 
            ...
            CheckIcon.Image = "rbxassetid://16932740082" 
    ...
    local function CreateTabButton(tabName, iconId, isConfigTab) 
        ...
        TabIcon.Image = iconId or "rbxassetid://16932740082" 
    ...
    -- Create Config tab
    local ConfigTabContainer, SwitchToConfigTab = CreateTabButton("⚙️ Configs", "rbxassetid://16932740082", true) 
