...
Components.Section = (function()
	...
	return function(Title, Parent)
		...
		-- Buat container seperti asli
		originalContainer = New("Frame", {
		...
		Section.ContainerWrapper = New("Frame", {
			Size = UDim2.new(1, 0, 0, 0),
			Position = UDim2.fromOffset(0, 24),
			BackgroundTransparency = 1,
		...
		-- Motor untuk animasi
		Section.SizeMotor = Flipper.SingleMotor.new(26)
		Section.WrapperMotor = Flipper.SingleMotor.new(26)

		-- Setup motors
		Section.SizeMotor:onStep(function(value)
			originalContainer.Size = UDim2.new(1, 0, 0, value)
		end)

		Section.WrapperMotor:onStep(function(value)
			Section.ContainerWrapper.Size = UDim2.new(1, 0, 0, value)
		...
		function Section:Toggle()
			...
			if Section.IsOpen then
				-- Buka section
				Section.SizeMotor:setGoal(Spring(originalContainer.Size.Y.Offset, { frequency = 6 }))
				Section.WrapperMotor:setGoal(Spring(originalContainer.Size.Y.Offset, { frequency = 6 }))
			else
				-- Tutup section
				Section.SizeMotor:setGoal(Spring(26, { frequency = 6 }))
				Section.WrapperMotor:setGoal(Spring(0, { frequency = 6 }))
			end
		end
		...
		Creator.AddSignal(Section.Layout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			...
			originalContainer.Size = UDim2.new(1, 0, 0, contentHeight)
			
			if Section.IsOpen then
				Section.SizeMotor:setGoal(Spring(contentHeight, { frequency = 6 }))
				Section.WrapperMotor:setGoal(Spring(contentHeight, { frequency = 6 }))
			end
		end)
		...
		if titleLabel then
			...
			titleLabel.TextButton = true
			Creator.AddSignal(titleLabel.MouseButton1Click, function()
				Section:Toggle()
			end)
		end

		-- Kembalikan container asli untuk kompatibilitas
		Section.Container = originalContainer
		
		-- Panggil update size awal
		task.spawn(function()
			task.wait(0.1)
			local contentHeight = Section.Layout.AbsoluteContentSize.Y
			if contentHeight > 0 then
				originalContainer.Size = UDim2.new(1, 0, 0, contentHeight)
				Section.ContainerWrapper.Size = UDim2.new(1, 0, 0, contentHeight)
