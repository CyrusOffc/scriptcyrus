...
-- Tambahkan ini ke file library Anda (tess1.lua) di bagian ElementsTable:
...
ElementsTable.TitleFitur = (function()
    ...
    function Element:New(Idx, Config)
        ...
        -- Container utama
        local Container = self.Container or self.Parent
        ...
        if TitleFitur.Icon then
            local iconImage = Library:GetIcon(TitleFitur.Icon)
            if iconImage then
        ...
        -- Calculate positions
        local titleXOffset = IconLabel and 40 or 12
        local titleWidth = IconLabel and -48 or -24

        -- Title text dengan font yang bagus
        local TitleLabel = New("TextLabel", {
            ...
            TextYAlignment = Enum.TextYAlignment.Center,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, titleWidth, 0, 22),
            Position = UDim2.fromOffset(titleXOffset, 11),
            ThemeTag = {
                TextColor3 = "Text",
            },
        })
        ...
        if TitleFitur.SubTitle ~= "" then
            SubTitleLabel = New("TextLabel", {
                ...
                TextYAlignment = Enum.TextYAlignment.Center,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, titleWidth, 0, 16),
                Position = UDim2.fromOffset(titleXOffset, 33),
                ThemeTag = {
                    TextColor3 = "SubText",
                },
            })
        ...
        local RightDecor = New("TextLabel", {
            Text = "▐",
            FontFace = Font.new("rbxasset://fonts/families/Arial.json", Enum.FontWeight.Bold),
            TextSize = 18,
            TextColor3 = Color3.fromRGB(100, 150, 255),
        ...
        -- Add all to frame
        Background.Parent = TitleFrame
        InnerStroke.Parent = Background
        TitleLabel.Parent = TitleFrame
        Separator.Parent = TitleFrame
        LeftDecor.Parent = TitleFrame
        RightDecor.Parent = TitleFrame
        ...
        local function updateSize()
            local hasSubtitle = TitleFitur.SubTitle ~= "" and SubTitleLabel
            local height = hasSubtitle and 56 or 44
            
            TitleFrame.Size = UDim2.new(1, 0, 0, height)
            Background.Size = UDim2.new(1, -12, 0, height)
            Separator.Position = UDim2.new(0, 12, 1, -1)
            
            if hasSubtitle then
                TitleLabel.Position = UDim2.fromOffset(titleXOffset, 8)
                SubTitleLabel.Position = UDim2.fromOffset(titleXOffset, 30)
                SubTitleLabel.Visible = true
            else
                TitleLabel.Position = UDim2.fromOffset(titleXOffset, 11)
                if SubTitleLabel then
        ...
        function TitleFitur:SetIcon(IconName)
            ...
            if iconImage then
                if not IconLabel then
                    ...
                    -- Update positions
                    local newTitleXOffset = 40
                    TitleLabel.Position = UDim2.fromOffset(newTitleXOffset, TitleLabel.Position.Y.Offset)
                    TitleLabel.Size = UDim2.new(1, -48, 0, 22)
                    
                    if SubTitleLabel then
                        SubTitleLabel.Position = UDim2.fromOffset(newTitleXOffset, SubTitleLabel.Position.Y.Offset)
                        SubTitleLabel.Size = UDim2.new(1, -48, 0, 16)
                    end
                else
            ...
            elseif IconLabel then
                ...
                -- Reset positions
                TitleLabel.Position = UDim2.fromOffset(12, TitleLabel.Position.Y.Offset)
                TitleLabel.Size = UDim2.new(1, -24, 0, 22)
                
                if SubTitleLabel then
                    SubTitleLabel.Position = UDim2.fromOffset(12, SubTitleLabel.Position.Y.Offset)
        ...
        TitleFitur.Frame = TitleFrame
        TitleFitur.Visible = TitleFitur.SetVisible
        TitleFitur.Elements = TitleFitur

        -- Add to Library options jika ada Idx
        if Idx and Library.Options then
