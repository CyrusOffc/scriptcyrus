...
ElementsTable.Slider = (function()
    ...
    function Element:New(Idx, Config)
        ...
        -- Motor untuk animasi dot
        Slider.DotMotor = Flipper.SingleMotor.new(0)
        
        -- Dot slider dengan efek hover
        ...
        -- Input box untuk nilai manual
        Slider.InputBox = New("TextBox", {
            FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"),
            Text = "",
            TextSize = 12,
            TextXAlignment = Enum.TextXAlignment.Center,
        ...
        -- Setup dot motor
        Slider.DotMotor:onStep(function(value)
            Slider.Dot.Position = UDim2.new(value, -9, 0.5, 0)
            Slider.Fill.Size = UDim2.fromScale(value, 1)
        end)

        -- Hover effect untuk dot
        local dotMotor, setDotSize = Creator.SpringMotor(1, Slider.Dot, "Size", false, false)
        
        Creator.AddSignal(Slider.Dot.MouseEnter, function()
            setDotSize(UDim2.fromOffset(22, 22))
        end)
        
        Creator.AddSignal(Slider.Dot.MouseLeave, function()
            if not Slider.Dragging then
                setDotSize(UDim2.fromOffset(18, 18))
            end
        end)

        -- Fungsi untuk update posisi berdasarkan mouse
        local function updateFromMouse(input)
            local railAbsPos = Slider.Rail.AbsolutePosition
            local railAbsSize = Slider.Rail.AbsoluteSize
            
            local mousePos = input.Position.X
            local relative = (mousePos - railAbsPos.X) / railAbsSize.X
        ...
        local function startDragging(input)
            ...
            -- Update langsung saat mulai drag
            local scale = updateFromMouse(input)
            Slider:SetValue(Slider.Min + ((Slider.Max - Slider.Min) * scale))
            
            -- Connection untuk drag
            local connection
            connection = RunService.RenderStepped:Connect(function()
                if Slider.Dragging then
                    ...
                    Slider:SetValue(Slider.Min + ((Slider.Max - Slider.Min) * clamped))
                else
                    connection:Disconnect()
                end
            end)
        ...
        -- Event untuk rail (click to slide)
        Creator.AddSignal(Slider.Rail.InputBegan, function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                startDragging(input)
        ...
        -- Event untuk stop dragging global
        Creator.AddSignal(UserInputService.InputEnded, function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                stopDragging()
            end
        ...
        -- Input box handling
        Creator.AddSignal(Slider.InputBox.Focused, function()
            Slider.InputBox.Text = tostring(Slider.Value)
        end)

        Creator.AddSignal(Slider.InputBox.FocusLost, function(enterPressed)
            if enterPressed then
                local text = Slider.InputBox.Text
        ...
        function Slider:SetValue(Value)
            ...
            if self.Value ~= rounded then
                ...
                -- Animate with spring
                Slider.DotMotor:setGoal(Flipper.Spring.new(scale, {
                    frequency = 10,
                    dampingRatio = 0.8
                }))
                
                -- Update display
                Slider.Display.Text = tostring(self.Value)
                Slider.InputBox.Text = tostring(self.Value)
        ...
        -- Add input box to frame
        Slider.InputBox.Parent = SliderFrame.Frame
        Slider.Display.Parent = Slider.Inner
