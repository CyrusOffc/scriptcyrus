...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		-- Container Layout untuk tab content
		local ContainerLayout = New("UIListLayout", {
			Padding = UDim.new(0, 5),
			SortOrder = Enum.SortOrder.LayoutOrder,
		})

		-- MAIN CONTAINER FRAME dengan efek transisi
		Tab.ContainerFrame = New("Frame", {
			Size = UDim2.fromScale(1, 1),
			BackgroundTransparency = 1,
			Parent = Window.ContainerHolder,
			Visible = false,
			ClipsDescendants = true, -- Penting untuk animasi slide
		})

		-- INNER FRAME untuk konten yang akan dianimasi
		Tab.InnerFrame = New("Frame", {
			Size = UDim2.fromScale(1, 1),
			BackgroundTransparency = 1,
			Parent = Tab.ContainerFrame,
			Position = UDim2.fromOffset(0, 0), -- Default position
		})
		...
		-- Motors untuk animasi transisi
		Tab.TransitionMotor = Flipper.GroupMotor.new({
			PositionX = 0,      -- Untuk slide horizontal
			Opacity = 1,        -- Untuk fade
			Scale = 1,          -- Untuk scale effect
		})

		Tab.TransitionMotor:onStep(function(values)
			-- Apply semua efek transisi
			...
			if Tab.InnerFrame:FindFirstChild("UIScale") then
				Tab.InnerFrame.UIScale.Scale = values.Scale
			end
		end)

		-- Tambah UIScale untuk efek zoom (optional)
		Tab.InnerFrame.UIScale = New("UIScale", {
			Parent = Tab.InnerFrame,
			Scale = 1,
		})

		-- Hover effect untuk tab button
	...
	function TabModule:SelectTab(Tab)
		...
		if currentTab and currentTab.InnerFrame then
			-- Animasi fade out dan geser ke kiri
		...
		newTab.TransitionMotor:setGoal({
			PositionX = Instant(30),  -- Mulai dari kanan
			Opacity = Instant(1),      -- Mulai transparan
			Scale = Instant(0.95),     -- Mulai kecil
		})

		-- Animasi masuk untuk tab baru
		task.wait(0.05) -- Delay kecil biar efek lebih dramatic
		newTab.TransitionMotor:setGoal({
			PositionX = Spring(0, { frequency = 10, dampingRatio = 0.7 }),  -- Geser ke posisi normal
			Opacity = Spring(0, { frequency = 12 }),                         -- Fade in
			Scale = Spring(1, { frequency = 9, dampingRatio = 0.8 }),        -- Scale ke normal
		})

		-- Efek tambahan: animasi untuk ContainerHolder
