...
ElementsTable.Slider = (function()
    ...
    function Element:New(Idx, Config)
        ...
        -- Motor untuk animasi smooth
        Slider.ValueMotor = Flipper.SingleMotor.new(0)
        Slider.DisplayMotor = Flipper.SingleMotor.new(0)

        -- Dot dengan efek yang bagus
        ...
        -- Container untuk rail
        Slider.RailContainer = New("Frame", {
        ...
        Slider.MaxLabel = New("TextLabel", {
            FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"),
            Text = tostring(Config.Max),
            TextSize = 11,
            TextXAlignment = Enum.TextXAlignment.Right,
            BackgroundTransparency = 1,
            Size = UDim2.new(0, 30, 0, 14),
            Position = UDim2.new(1, -32, 1, 2),
            AnchorPoint = Vector2.new(1, 0),
            ThemeTag = {
                TextColor3 = "SubText",
            },
        ...
        Slider.Inner = New("Frame", {
            ...
            New("UISizeConstraint", {
                ...
                MinSize = Vector2.new(100, 30),
            }),
            Slider.RailContainer,
            Slider.ValueDisplay,
            Slider.MinLabel,
            Slider.MaxLabel,
        })
        ...
        -- Setup motors
        Slider.ValueMotor:onStep(function(value)
            local railWidth = Slider.RailContainer.AbsoluteSize.X
            local dotPosition = value * railWidth
            
            -- Update dot position dengan smooth
            Slider.Dot.Position = UDim2.new(0, dotPosition - 10, 0.5, 0)
            
            -- Update fill width
            Slider.Fill.Size = UDim2.new(value, 0, 1, 0)
            
            -- Update display value dengan spring
            local displayValue = Slider.Min + ((Slider.Max - Slider.Min) * value)
            Slider.DisplayMotor:setGoal(Flipper.Spring.new(displayValue, {
                frequency = 15,
                dampingRatio = 0.7
            }))
        end)

        Slider.DisplayMotor:onStep(function(value)
            local rounded = Library:Round(value, Slider.Rounding)
            Slider.ValueDisplay.Text = tostring(rounded)
        end)

        -- Hover effect untuk dot
        local dotSizeMotor, setDotSize = Creator.SpringMotor(1, Slider.Dot, "Size")
        
        Creator.AddSignal(Slider.Dot.MouseEnter, function()
            setDotSize(UDim2.fromOffset(24, 24))
        end)
        
        Creator.AddSignal(Slider.Dot.MouseLeave, function()
            if not Slider.Dragging then
                setDotSize(UDim2.fromOffset(20, 20))
            end
        end)

        -- Hover effect untuk rail
        local railMotor, setRailHeight = Creator.SpringMotor(8, Slider.RailContainer, "Size", false, false)
        
        Creator.AddSignal(Slider.RailContainer.MouseEnter, function()
            setRailHeight(UDim2.new(1, 0, 0, 10))
        end)
        
        Creator.AddSignal(Slider.RailContainer.MouseLeave, function()
            if not Slider.Dragging then
                setRailHeight(UDim2.new(1, 0, 0, 8))
            end
        end)
        ...
        local function startDragging()
            ...
            -- Play sound/feedback
            -- (optional) bisa tambahkan sound effect di sini
            
            -- Connection untuk smooth dragging
            local connection
            connection = RunService.RenderStepped:Connect(function()
                if Slider.Dragging then
                    local mouse = UserInputService:GetMouseLocation()
                    local railAbsPos = Slider.RailContainer.AbsolutePosition
                    local railAbsSize = Slider.RailContainer.AbsoluteSize
                    
                    -- Hitung posisi relatif mouse di rail
                    local relative = (mouse.X - railAbsPos.X) / railAbsSize.X
                    local clamped = math.clamp(relative, 0, 1)
                    
                    -- Update motor dengan spring untuk smoothing
                    Slider.ValueMotor:setGoal(Flipper.Spring.new(clamped, {
                        frequency = 20,
                        dampingRatio = 0.6
                    }))
                    
                    -- Hitung value aktual
                    local value = Slider.Min + ((Slider.Max - Slider.Min) * clamped)
                    local rounded = Library:Round(value, Slider.Rounding)
                    
                    -- Update value jika berubah
                    if Slider.Value ~= rounded then
        ...
        local function stopDragging()
            if Slider.Dragging then
                ...
                -- Finalize value dengan rounding
                if Slider.Value then
                    Slider.Value = Library:Round(Slider.Value, Slider.Rounding)
        ...
        -- Event untuk dot
        Creator.AddSignal(Slider.Dot.InputBegan, function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                startDragging()
            end
        end)

        -- Event untuk rail (click to jump)
        Creator.AddSignal(Slider.RailContainer.InputBegan, function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                -- Langsung set ke posisi klik
                local railAbsPos = Slider.RailContainer.AbsolutePosition
                local railAbsSize = Slider.RailContainer.AbsoluteSize
                ...
                -- Mulai dragging setelah klik
                startDragging()
            end
        end)

        -- Global mouse up untuk stop dragging
        Creator.AddSignal(UserInputService.InputEnded, function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                stopDragging()
            end
        ...
        -- Double click untuk reset ke default
        local lastClickTime = 0
        Creator.AddSignal(Slider.RailContainer.MouseButton1Click, function()
        ...
        function Slider:SetValue(Value)
            ...
            if self.Value ~= rounded then
                ...
                -- Animate dengan spring yang smooth
                Slider.ValueMotor:setGoal(Flipper.Spring.new(scale, {
                    frequency = 15,
                    dampingRatio = 0.7
                }))
        ...
        function Slider:SetRange(Min, Max)
            if Min then
                Slider.Min = Min
                Slider.MinLabel.Text = tostring(Min)
            end
            if Max then
                Slider.Max = Max
                Slider.MaxLabel.Text = tostring(Max)
            end
            
            -- Re-clamp current value
            Slider:SetValue(Slider.Value or Config.Default)
        end

        function Slider:GetPercentage()
            if not Slider.Value then return 0 end
        ...
        -- Set initial value dengan animasi
        task.spawn(function()
            task.wait(0.1)
            Slider:SetValue(Config.Default)
        end)
