...
Components.Section = (function()
    ...
    return function(Title, Parent)
        ...
        -- Tombol toggle dengan icon yang lebih bagus
        ...
        -- Inner stroke (tipis untuk efek grid/outline)
        ...
        -- Fungsi untuk update ukuran section
        ...
        -- Fungsi toggle dengan animasi icon
        function Section:Toggle()
            ...
            local tween = TweenService:Create(
                Section.ToggleButton,
                tweenInfo,
                { Rotation = Section.IsOpen and 0 or -90 }
            )
            tween:Play()
        ...
        -- Klik title juga bisa toggle
        Section.TitleLabel.Active = true
        Creator.AddSignal(Section.TitleLabel.MouseButton1Click, function()
            Section:Toggle()
        end)

        -- Hover effect untuk title label
        local titleMotor, setTitleTrans = Creator.SpringMotor(0, Section.TitleLabel, "TextTransparency")
        Creator.AddSignal(Section.TitleLabel.MouseEnter, function()
            setTitleTrans(0.3)
        end)
        Creator.AddSignal(Section.TitleLabel.MouseLeave, function()
            setTitleTrans(0)
        end)

        -- Update size awal setelah semua element ditambahkan
        task.spawn(function()
            task.wait(0.3)  -- Tunggu semua element selesai ditambahkan
            updateSize()
        end)

        -- Fungsi untuk mengubah title
        function Section:SetTitle(NewTitle)
            Section.TitleLabel.Text = NewTitle
        end

        -- Fungsi untuk mengubah visibility
        function Section:SetVisible(Visible)
            Section.Root.Visible = Visible
        end

        -- Fungsi untuk destroy
